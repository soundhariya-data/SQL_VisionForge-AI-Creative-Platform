...
2)What is the total content created, and total revenue, across the platform?
SELECT
  SUM(a.Content_created) AS total_content_created,
  SUM(b.amount_billed)   AS total_revenue
FROM activity a JOIN billing_transactions b
ON b.user_id = a.user_id;
#Overall platform output and overall revenue, before any segmentation.

3)How many users are on each plan?
select count(user_id),u.plan_id,plan_name
from users u left join plans p
on u.plan_id=p.plan_id
group by plan_id;

4)Usage-Based Customer Segmentation
select customer_group,sum(content_Created) TOTAL_CONTENT,CASE WHEN sum(content_Created)>10 then "HIGH USAGE"
...
#To understand how actively different customers  groups are using the product and target engagement efforts accordingly
#High Usage → understand what keeps these customers groups engaged and encourage continued usage.
